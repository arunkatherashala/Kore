// kore-self — Auto-generated: Status Engine — analyzes 'status' patterns in your memory
...
pub struct StatusEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct StatusEngine {
    pub patterns:       Vec<StatusEnginePattern>,
...
impl StatusEngine {
    ...
    /// Analyze all memories for 'status'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("status");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(StatusEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[StatusEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
