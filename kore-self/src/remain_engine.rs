// kore-self — Auto-generated: Remain Engine — analyzes 'remain' patterns in your memory
...
pub struct RemainEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RemainEngine {
    pub patterns:       Vec<RemainEnginePattern>,
...
impl RemainEngine {
    ...
    /// Analyze all memories for 'remain'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("remain");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(RemainEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[RemainEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
