// kore-self — Auto-generated: Understanding Engine — analyzes 'understanding' patterns in your memory
...
pub struct UnderstandingEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct UnderstandingEngine {
    pub patterns:       Vec<UnderstandingEnginePattern>,
...
impl UnderstandingEngine {
    ...
    /// Analyze all memories for 'understanding'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("understanding");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(UnderstandingEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[UnderstandingEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
