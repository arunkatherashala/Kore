// kore-self — Auto-generated: Datablock Engine — analyzes 'datablock' patterns in your memory
...
pub struct DatablockEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DatablockEngine {
    pub patterns:       Vec<DatablockEnginePattern>,
    pub total_analyzed: u64,
    pub last_run:       String,
}

impl DatablockEngine {
    pub fn new() -> Self {
        Self {
            patterns:       vec![],
            total_analyzed: 0,
    ...
    /// Analyze all memories for 'datablock'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("datablock");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(DatablockEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[DatablockEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for DatablockEngine {
