// kore-self — Auto-generated: Current Engine — analyzes 'current' patterns in your memory
...
pub struct CurrentEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct CurrentEngine {
    pub patterns:       Vec<CurrentEnginePattern>,
...
impl CurrentEngine {
    ...
    /// Analyze all memories for 'current'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("current");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(CurrentEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[CurrentEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for CurrentEngine {
