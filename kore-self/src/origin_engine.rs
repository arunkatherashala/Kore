// kore-self — Auto-generated: Origin Engine — analyzes 'origin' patterns in your memory
...
pub struct OriginEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct OriginEngine {
    pub patterns:       Vec<OriginEnginePattern>,
...
impl OriginEngine {
    ...
    /// Analyze all memories for 'origin'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("origin");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(OriginEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[OriginEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
