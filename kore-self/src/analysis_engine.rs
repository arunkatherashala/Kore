// kore-self — Auto-generated: Analysis Engine — analyzes 'analysis' patterns in your memory
...
pub struct AnalysisEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct AnalysisEngine {
    pub patterns:       Vec<AnalysisEnginePattern>,
...
impl AnalysisEngine {
    ...
    /// Analyze all memories for 'analysis'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("analysis");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(AnalysisEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[AnalysisEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
