// kore-self — Auto-generated: Produced Engine — analyzes 'produced' patterns in your memory
...
pub struct ProducedEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProducedEngine {
    pub patterns:       Vec<ProducedEnginePattern>,
...
impl ProducedEngine {
    ...
    /// Analyze all memories for 'produced'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("produced");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ProducedEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ProducedEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for ProducedEngine {
