// kore-self — Auto-generated: Action Engine — analyzes 'action' patterns in your memory
...
pub struct ActionEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct ActionEngine {
    pub patterns:       Vec<ActionEnginePattern>,
...
impl ActionEngine {
    ...
    /// Analyze all memories for 'action'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("action");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ActionEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ActionEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
