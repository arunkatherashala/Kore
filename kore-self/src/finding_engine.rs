// kore-self — Auto-generated: Finding Engine — analyzes 'finding' patterns in your memory
...
pub struct FindingEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct FindingEngine {
    pub patterns:       Vec<FindingEnginePattern>,
...
impl FindingEngine {
    ...
    /// Analyze all memories for 'finding'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("finding");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(FindingEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[FindingEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
