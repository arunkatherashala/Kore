// kore-self — Auto-generated: Point Engine — analyzes 'point' patterns in your memory
...
pub struct PointEngine {
    pub patterns:       Vec<PointEnginePattern>,
...
impl PointEngine {
    ...
    /// Analyze all memories for 'point'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("point");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(PointEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[PointEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
