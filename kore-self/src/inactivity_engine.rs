// kore-self — Auto-generated: Inactivity Engine — analyzes 'inactivity' patterns in your memory
...
pub struct InactivityEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct InactivityEngine {
    pub patterns:       Vec<InactivityEnginePattern>,
...
impl InactivityEngine {
    ...
    /// Analyze all memories for 'inactivity'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("inactivity");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(InactivityEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[InactivityEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for InactivityEngine {
