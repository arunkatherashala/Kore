// kore-self — Auto-generated: Dominant Engine — analyzes 'dominant' patterns in your memory
...
pub struct DominantEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DominantEngine {
    pub patterns:       Vec<DominantEnginePattern>,
...
impl DominantEngine {
    ...
    /// Analyze all memories for 'dominant'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("dominant");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(DominantEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[DominantEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for DominantEngine {
