// kore-self — Auto-generated: Become Engine — analyzes 'become' patterns in your memory
...
pub struct BecomeEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BecomeEngine {
    pub patterns:       Vec<BecomeEnginePattern>,
...
impl BecomeEngine {
    ...
    /// Analyze all memories for 'become'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("become");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(BecomeEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[BecomeEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
