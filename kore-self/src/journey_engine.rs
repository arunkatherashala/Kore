// kore-self — Auto-generated: Journey Engine — analyzes 'journey' patterns in your memory
...
pub struct JourneyEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct JourneyEngine {
    pub patterns:       Vec<JourneyEnginePattern>,
...
impl JourneyEngine {
    ...
    /// Analyze all memories for 'journey'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("journey");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(JourneyEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[JourneyEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
