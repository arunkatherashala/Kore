// kore-self — Auto-generated: Explore Engine — analyzes 'explore' patterns in your memory
...
pub struct ExploreEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct ExploreEngine {
    pub patterns:       Vec<ExploreEnginePattern>,
...
impl ExploreEngine {
    ...
    /// Analyze all memories for 'explore'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("explore");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ExploreEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ExploreEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for ExploreEngine {
