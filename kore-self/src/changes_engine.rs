// kore-self — Auto-generated: Changes Engine — analyzes 'changes' patterns in your memory
...
pub struct ChangesEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ChangesEngine {
    pub patterns:       Vec<ChangesEnginePattern>,
...
impl ChangesEngine {
    ...
    /// Analyze all memories for 'changes'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("changes");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ChangesEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ChangesEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for ChangesEngine {
