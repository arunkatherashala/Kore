// kore-self — Auto-generated: Understand Engine — analyzes 'understand' patterns in your memory
...
pub struct UnderstandEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UnderstandEngine {
    pub patterns:       Vec<UnderstandEnginePattern>,
    pub total_analyzed: u64,
    pub last_run:       String,
}

impl UnderstandEngine {
    pub fn new() -> Self {
        Self {
            patterns:       vec![],
            total_analyzed: 0,
    ...
    /// Analyze all memories for 'understand'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("understand");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(UnderstandEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[UnderstandEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for UnderstandEngine {
