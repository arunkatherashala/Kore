// kore-self — Auto-generated: Through Engine — analyzes 'through' patterns in your memory
...
pub struct ThroughEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ThroughEngine {
    pub patterns:       Vec<ThroughEnginePattern>,
...
impl ThroughEngine {
    ...
    /// Analyze all memories for 'through'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("through");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ThroughEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ThroughEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for ThroughEngine {
