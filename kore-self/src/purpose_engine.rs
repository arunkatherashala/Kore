// kore-self — Auto-generated: Purpose Engine — analyzes 'purpose' patterns in your memory
...
pub struct PurposeEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PurposeEngine {
    pub patterns:       Vec<PurposeEnginePattern>,
...
impl PurposeEngine {
    ...
    /// Analyze all memories for 'purpose'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("purpose");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(PurposeEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[PurposeEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for PurposeEngine {
