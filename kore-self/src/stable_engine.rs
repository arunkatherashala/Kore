// kore-self — Auto-generated: Stable Engine — analyzes 'stable' patterns in your memory
...
pub struct StableEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct StableEngine {
    pub patterns:       Vec<StableEnginePattern>,
...
impl StableEngine {
    ...
    /// Analyze all memories for 'stable'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("stable");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(StableEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[StableEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
