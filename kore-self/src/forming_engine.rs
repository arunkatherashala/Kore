// kore-self — Auto-generated: Forming Engine — analyzes 'forming' patterns in your memory
...
pub struct FormingEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FormingEngine {
    pub patterns:       Vec<FormingEnginePattern>,
...
impl FormingEngine {
    ...
    /// Analyze all memories for 'forming'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("forming");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(FormingEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[FormingEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for FormingEngine {
