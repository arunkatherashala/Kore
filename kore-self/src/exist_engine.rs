// kore-self — Auto-generated: Exist Engine — analyzes 'exist' patterns in your memory
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ExistEngine {
    pub patterns:       Vec<ExistEnginePattern>,
...
impl ExistEngine {
    ...
    /// Analyze all memories for 'exist'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("exist");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ExistEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ExistEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
