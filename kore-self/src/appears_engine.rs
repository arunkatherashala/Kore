// kore-self — Auto-generated: Appears Engine — analyzes 'appears' patterns in your memory
...
pub struct AppearsEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AppearsEngine {
    pub patterns:       Vec<AppearsEnginePattern>,
...
impl AppearsEngine {
    ...
    /// Analyze all memories for 'appears'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("appears");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(AppearsEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[AppearsEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for AppearsEngine {
