// kore-self — Auto-generated: Problem Engine — tracks recurring errors and resolution patterns
...
pub struct ProblemEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct ProblemEngine {
    pub patterns:       Vec<ProblemEnginePattern>,
...
impl ProblemEngine {
    ...
    /// Analyze all memories for 'failed'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("failed");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ProblemEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ProblemEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
