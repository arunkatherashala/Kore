// kore-self — Auto-generated: Experience Engine — analyzes 'experience' patterns in your memory
...
pub struct ExperienceEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct ExperienceEngine {
    pub patterns:       Vec<ExperienceEnginePattern>,
...
impl ExperienceEngine {
    ...
    /// Analyze all memories for 'experience'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("experience");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ExperienceEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ExperienceEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
