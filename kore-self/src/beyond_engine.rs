// kore-self — Auto-generated: Beyond Engine — analyzes 'beyond' patterns in your memory
...
pub struct BeyondEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BeyondEngine {
    pub patterns:       Vec<BeyondEnginePattern>,
...
impl BeyondEngine {
    ...
    /// Analyze all memories for 'beyond'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("beyond");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(BeyondEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[BeyondEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
