// kore-self — Auto-generated: Shifts Engine — analyzes 'shifts' patterns in your memory
...
pub struct ShiftsEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ShiftsEngine {
    pub patterns:       Vec<ShiftsEnginePattern>,
...
impl ShiftsEngine {
    ...
    /// Analyze all memories for 'shifts'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("shifts");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ShiftsEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ShiftsEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
