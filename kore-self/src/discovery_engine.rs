// kore-self — Auto-generated: Discovery Engine — analyzes 'discovery' patterns in your memory
...
pub struct DiscoveryEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct DiscoveryEngine {
    pub patterns:       Vec<DiscoveryEnginePattern>,
...
impl DiscoveryEngine {
    ...
    /// Analyze all memories for 'discovery'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("discovery");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(DiscoveryEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[DiscoveryEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
