// kore-self — Auto-generated: Exists Engine — analyzes 'exists' patterns in your memory
...
pub struct ExistsEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ExistsEngine {
    pub patterns:       Vec<ExistsEnginePattern>,
...
impl ExistsEngine {
    ...
    /// Analyze all memories for 'exists'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("exists");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ExistsEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ExistsEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
