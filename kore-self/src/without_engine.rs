// kore-self — Auto-generated: Without Engine — analyzes 'without' patterns in your memory
...
pub struct WithoutEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct WithoutEngine {
    pub patterns:       Vec<WithoutEnginePattern>,
...
impl WithoutEngine {
    ...
    /// Analyze all memories for 'without'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("without");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(WithoutEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[WithoutEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for WithoutEngine {
