// kore-self — Auto-generated: Accumulated Engine — analyzes 'accumulated' patterns in your memory
...
pub struct AccumulatedEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AccumulatedEngine {
    pub patterns:       Vec<AccumulatedEnginePattern>,
    pub total_analyzed: u64,
    pub last_run:       String,
}

impl AccumulatedEngine {
    pub fn new() -> Self {
        Self {
            patterns:       vec![],
            total_analyzed: 0,
    ...
    /// Analyze all memories for 'accumulated'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("accumulated");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(AccumulatedEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[AccumulatedEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for AccumulatedEngine {
