// kore-self — Auto-generated: Improve Engine — analyzes 'improve' patterns in your memory
...
pub struct ImproveEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ImproveEngine {
    pub patterns:       Vec<ImproveEnginePattern>,
...
impl ImproveEngine {
    ...
    /// Analyze all memories for 'improve'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("improve");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ImproveEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ImproveEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for ImproveEngine {
