// kore-self — Auto-generated: Happened Engine — analyzes 'happened' patterns in your memory
...
pub struct HappenedEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HappenedEngine {
    pub patterns:       Vec<HappenedEnginePattern>,
...
impl HappenedEngine {
    ...
    /// Analyze all memories for 'happened'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("happened");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(HappenedEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[HappenedEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for HappenedEngine {
