// kore-self — Auto-generated: Medium Engine — analyzes 'medium' patterns in your memory
...
pub struct MediumEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MediumEngine {
    pub patterns:       Vec<MediumEnginePattern>,
...
impl MediumEngine {
    ...
    /// Analyze all memories for 'medium'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("medium");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(MediumEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[MediumEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
