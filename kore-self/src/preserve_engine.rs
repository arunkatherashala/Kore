// kore-self — Auto-generated: Preserve Engine — analyzes 'preserve' patterns in your memory
...
pub struct PreserveEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PreserveEngine {
    pub patterns:       Vec<PreserveEnginePattern>,
...
impl PreserveEngine {
    ...
    /// Analyze all memories for 'preserve'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("preserve");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(PreserveEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[PreserveEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for PreserveEngine {
