// kore-self — Auto-generated: Columns Engine — analyzes 'columns' patterns in your memory
...
pub struct ColumnsEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ColumnsEngine {
    pub patterns:       Vec<ColumnsEnginePattern>,
...
impl ColumnsEngine {
    ...
    /// Analyze all memories for 'columns'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("columns");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ColumnsEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ColumnsEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for ColumnsEngine {
