// kore-self — Auto-generated: Theories Engine — analyzes 'theories' patterns in your memory
...
pub struct TheoriesEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TheoriesEngine {
    pub patterns:       Vec<TheoriesEnginePattern>,
...
impl TheoriesEngine {
    ...
    /// Analyze all memories for 'theories'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("theories");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(TheoriesEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[TheoriesEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for TheoriesEngine {
