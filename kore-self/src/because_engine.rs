// kore-self — Auto-generated: Because Engine — analyzes 'because' patterns in your memory
...
pub struct BecauseEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BecauseEngine {
    pub patterns:       Vec<BecauseEnginePattern>,
...
impl BecauseEngine {
    ...
    /// Analyze all memories for 'because'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("because");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(BecauseEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[BecauseEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
...
impl Default for BecauseEngine {
