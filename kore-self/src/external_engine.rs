// kore-self — Auto-generated: External Engine — analyzes 'external' patterns in your memory
...
pub struct ExternalEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct ExternalEngine {
    pub patterns:       Vec<ExternalEnginePattern>,
...
impl ExternalEngine {
    ...
    /// Analyze all memories for 'external'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("external");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(ExternalEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[ExternalEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
