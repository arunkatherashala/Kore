// kore-self — Auto-generated: Theory Engine — analyzes 'theory' patterns in your memory
...
pub struct TheoryEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct TheoryEngine {
    pub patterns:       Vec<TheoryEnginePattern>,
...
impl TheoryEngine {
    ...
    /// Analyze all memories for 'theory'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("theory");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(TheoryEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[TheoryEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
