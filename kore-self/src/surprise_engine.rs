// kore-self — Auto-generated: Surprise Engine — analyzes 'surprise' patterns in your memory
...
pub struct SurpriseEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct SurpriseEngine {
    pub patterns:       Vec<SurpriseEnginePattern>,
...
impl SurpriseEngine {
    ...
    /// Analyze all memories for 'surprise'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("surprise");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(SurpriseEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[SurpriseEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
