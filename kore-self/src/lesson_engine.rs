// kore-self — Auto-generated: Lesson Engine — analyzes 'lesson' patterns in your memory
...
pub struct LessonEnginePattern {
    pub topic:      String,
    pub count:      u32,
    pub weight:     f64,
...
pub struct LessonEngine {
    pub patterns:       Vec<LessonEnginePattern>,
...
impl LessonEngine {
    ...
    /// Analyze all memories for 'lesson'-related patterns.
    pub fn analyze(&mut self, memories: &[Memory]) -> Vec<String> {
        if memories.is_empty() { return vec![]; }

        let keywords = expand_keywords("lesson");
        ...
        for (topic, (count, weight)) in &freq {
            self.patterns.push(LessonEnginePattern {
                topic:     topic.clone(),
                count:     *count,
        ...
        self.patterns.iter().take(3).map(|p| {
            format!("[LessonEngine] '{}' detected {} times (avg importance {:.2})", p.topic, p.count, p.weight)
        }).collect()
    }
